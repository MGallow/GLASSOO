...
#' \item{maxit1}{maximum number of iterations for outer (blockwise) loop.}
#' \item{maxit2}{maximum number of iterations for inner (lasso) loop.}
...
GLASSO = function(X = NULL, S = NULL, lam = 0.1, diagonal = FALSE, crit1 = c("max", "loglik"), crit2 = c("loss", "max"), tol1 = 1e-04, tol2 = 1e-04, maxit1 = 10000, maxit2 = 10000) {
  ...
  if (!(all(c(tol1, tol2, maxit1, maxit2) > 0))) {
    stop("Entry must be positive!")
  }
  if (all(c(maxit1, maxit2)%%1 != 0)) {
    stop("Entry must be an integer!")
  }
  
  # match values
  crit1 = match.arg(crit1)
  crit2 = match.arg(crit2)
  lam = sort(lam)
  call = match.call()
  ...
  GLASSO = GLASSOc(S = S, initSigma = init, initOmega = init, lam = lam, diagonal = diagonal, crit1 = crit1, crit2 = crit2, tol1 = tol1, tol2 = tol2, maxit1 = maxit1, maxit2 = maxit2)
  ...
  # compute loglik
  n = ifelse(is.null(X), nrow(S), nrow(X))
  loglik = (-n/2) * (sum(GLASSO$Omega * S) - determinant(GLASSO$Omega, logarithm = TRUE)$modulus[1] + GLASSO$lam * sum(abs(C * GLASSO$Omega)))
  
  
  # return values
  tuning = matrix(c(log10(lam), lam), ncol = 2)
  colnames(tuning) = c("log10(lam)", "alpha")
  returns = list(Call = call, Iterations = GLASSO$Iterations, Lambdas = GLASSO$lam, maxit1 = maxit1, maxit2 = maxit2, Omega = GLASSO$Omega, Sigma = GLASSO$Sigma, Loglik = loglik)
